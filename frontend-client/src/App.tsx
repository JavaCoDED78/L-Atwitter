...
const App: FC = (): ReactElement => {
  ...
  const [colorScheme, setColorScheme] = useState<ThemeOptions>(
    blueColor as ThemeOptions
  );
  ...
  useEffect(() => {
    ...
    processColorScheme(
      color !== null ? (color as ColorScheme) : ColorScheme.BLUE
    );
    ...
    if (myProfile) {
      stompClient.connect({}, () => {
        ...
        stompClient?.subscribe(
          "/topic/notifications/" + myProfile.id,
          (response) => {
      ...
      <div className="App">
        ...
        >
          <Switch location={background || location}>
            ...
            <Route
              path="/user/followers_you_follow/:id"
              component={FollowersYouKnow}
              exact
            />
            <Route
              path="/user/:id/:follow"
          ...
          {background && (
            <Route path="/modal/:id" children={<TweetImageModal />} />
          )}
        </Layout>
      </div>
    </MuiThemeProvider>
