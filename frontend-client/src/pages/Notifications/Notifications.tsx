...
import {
  Notification,
  NotificationType,
} from "../../store/ducks/notifications/contracts/state";
                    ...
                    <div style={{ flex: 1 }}>
                      {tweetAuthors.slice(0, 6).map((tweetAuthor) => (
                        <div
                          ...
                          onMouseEnter={() =>
                            handleHoverPopperWithUser!(tweetAuthor)
                          }
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <a
                        ...
                        onClick={(event) =>
                          handleClickUser(notification.user.id!, event)
                        }
...
const NotificationWithLink: FC<NotificationWithLinkProps> = ({
  notification,
  children,
}) => {
  return (
    <div>
      {notification.notificationType !== NotificationType.FOLLOW ? (
        ...
        <Link to={`/user/${notification.user.id}`}>{children}</Link>
      )}
    </div>
  );
};
