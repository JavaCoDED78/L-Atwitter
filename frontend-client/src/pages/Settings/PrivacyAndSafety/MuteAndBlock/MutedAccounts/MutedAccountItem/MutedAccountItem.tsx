...
const MutedAccountItem: FC<MutedAccountItemProps> = ({
  mutedUser,
}): ReactElement => {
  const dispatch = useDispatch();
  const myProfile = useSelector(selectUserData);
  const isUserMuted =
    myProfile?.userMutedList?.findIndex((user) => user.id === mutedUser?.id) !==
    -1;
    ...
    <Paper className={classes.container}>
      <Link to={`/user/${mutedUser?.id}`} className={classes.link}>
        <Avatar
          ...
          src={
            mutedUser?.avatar?.src ? mutedUser?.avatar.src : DEFAULT_PROFILE_IMG
          }
        />
      </Link>
      <div style={{ flex: 1 }}>
        <div className={classes.userInfoWrapper}>
          ...
          <div className={classes.muteButton}>
            ...
            >
              ...
              <HoverAction
                visible={visibleCloseAction}
                actionText={isUserMuted ? "Unmute" : "Mute"}
              />
            </IconButton>
          </div>
        </div>
        <Typography display="block">{mutedUser?.about}</Typography>
      </div>
      <ActionSnackbar
