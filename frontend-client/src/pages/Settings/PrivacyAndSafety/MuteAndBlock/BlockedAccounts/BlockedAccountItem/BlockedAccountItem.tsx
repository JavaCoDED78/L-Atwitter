...
const BlockedAccountItem: FC<BlockedAccountItemProps> = ({
  blockedUser,
}): ReactElement => {
      ...
      <div style={{ flex: 1 }}>
        <div className={classes.userInfoWrapper}>
          ...
          <div className={classes.blockButton}>
            <Button onClick={unblockUser} color="primary" variant="contained">
              {isUserBlocked ? "Blocked" : "Block"}
            </Button>
          </div>
        </div>
        <Typography display="block">{blockedUser?.about}</Typography>
      </div>
      <ActionSnackbar
