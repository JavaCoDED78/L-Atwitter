...
const UserPage: FC<SnackbarProps & HoverActionProps> = ({
  ...
  const isSubscriber =
    userProfile?.subscribers?.findIndex(
      (subscriber) => subscriber.id === myProfile?.id
    ) !== -1;
  const isFollower =
  ...
  const isUserMuted =
    myProfile?.userMutedList?.findIndex(
      (mutedUser) => mutedUser.id === userProfile?.id
    ) !== -1;
  const isUserBlocked =
    myProfile?.userBlockedList?.findIndex(
      (blockedUser) => blockedUser.id === userProfile?.id
    ) !== -1;
  const isMyProfileBlocked =
    userProfile?.userBlockedList?.findIndex(
      (blockedUser) => blockedUser.id === myProfile?.id
    ) !== -1;
  const isWaitingForApprove =
    userProfile?.followerRequests?.findIndex(
      (blockedUser) => blockedUser.id === myProfile?.id
    ) !== -1;
  ...
  useEffect(() => {
    ...
    stompClient.connect({}, () => {
      stompClient?.subscribe(
        "/topic/user/add/tweet/" + params.id,
        (response) => {
          dispatch(setAddedUserTweet(JSON.parse(response.body)));
        }
      );
      stompClient?.subscribe(
        "/topic/user/update/tweet/" + params.id,
        (response) => {
  ...
  useEffect(() => {
    setBtnText(
      isWaitingForApprove ? "Pending" : isUserBlocked ? "Blocked" : "Following"
    );
  ...
  useEffect(() => {
    if (isMyProfileLoaded && isUserProfileSuccessLoaded) {
      const followers = myProfile?.followers?.filter(({ id: id1 }) =>
        userProfile?.followers?.some(({ id: id2 }) => id2 === id1)
      );
  ...
  const onMuteUser = (): void => {
    ...
    setSnackBarMessage!(
      `@${userProfile?.username} has been ${isUserMuted ? "unmuted" : "muted"}.`
    );
        ...
        <div style={{ paddingTop: 53 }}>
          ...
          <div className={classes.info}>
            ...
            {userProfile !== undefined &&
              (isMyProfileBlocked ? null : userProfile?.id === myProfile?.id ? (
                <Button
                  onClick={
            ...
            {isMyProfileBlocked || userProfile?.privateProfile
              ? null
              : userProfile !== undefined && (
                  <FollowerGroup
                    user={userProfile}
          ...
          {userProfile === undefined ? (
            ...
            <div className={classes.privateProfileInfo}>
              <Typography
                component={"div"}
                className={classes.privateProfileInfoTitle}
              >
                You’re blocked
              </Typography>
              <Typography
                component={"div"}
                className={classes.privateProfileInfoText}
              >
                {`You can’t follow or see @${userProfile?.username}’s Tweets.`}{" "}
                <a
                  href={
                    "https://help.twitter.com/using-twitter/someone-blocked-me-on-twitter"
                  }
                  target={"_blank"}
