...
const ManageMembersSuggested: FC<ManageMembersSuggestedProps> = ({
  list,
}): ReactElement => {
  ...
  const handleClickSearch = (event: FormEvent<HTMLFormElement>): void => {
    ...
    UserApi.searchUsersByUsername(encodeURIComponent(text)).then((response) =>
      setUsers(response!)
    );
  };
  ...
  return (
    <div className={classes.container}>
      ...
      {users.length !== 0 ? (
        <>
          {users.map((user) => (
            <ManageMembersItem item={list} member={user} />
          ))}
        </>
      ) : (
