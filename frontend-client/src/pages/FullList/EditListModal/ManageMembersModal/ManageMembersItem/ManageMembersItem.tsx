...
const ManageMembersItem: FC<ManageMembersItemProps> = ({
  list,
  member,
}): ReactElement => {
  ...
  const isMember = list.members?.findIndex(
    (listMember) => listMember.id === member.id
  );
        ...
        <div className={classes.header}>
          <Link to={`/user/${member.id}`} className={classes.link}>
            ...
            >
              ...
              <Typography className={classes.about}>{member?.about}</Typography>
            </div>
          </Link>
          <div>
            {isMember === -1 ? (
