...
describe("userDetailSaga:", () => {
  describe("fetchUserDetailRequest:", () => {
    ...
    const worker = fetchUserDetailRequest(
      fetchUserDetail({ userId: 1, cancelTokenSource: cancelTokenSource })
    );
    ...
    testSetResponse({
      worker: worker,
      mockData: mockUserDetailResponse,
      action: setUserDetail,
      payload: mockUserDetailResponse,
  ...
  testWatchSaga(userDetailSaga, [
    {
      actionType: UserDetailActionsType.FETCH_USER_DETAIL,
      workSaga: fetchUserDetailRequest,
    },
  ]);
});
