...
export function* fetchUserListsByIdRequest({
  payload,
}: FetchUserListsByIdActionInterface) {
  try {
    yield put(setListsLoadingState(LoadingStatus.LOADING));
...
export function* addUserToListsRequest({
  payload,
}: AddUserToListsActionInterface) {
  try {
...
export function* processListMemberRequest({
  payload,
}: ProcessListMemberActionInterface) {
  try {
...
export function* listsSaga() {
  ...
  yield takeEvery(
    ListsActionType.FETCH_USER_LISTS_BY_ID,
    fetchUserListsByIdRequest
  );
  yield takeEvery(
    ListsActionType.FETCH_TWEET_LISTS_WHICH_USER_IN,
  ...
  yield takeEvery(
    ListsActionType.PROCESS_LIST_MEMBER,
