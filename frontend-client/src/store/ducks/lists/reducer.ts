...
export const listsReducer = produce(
  (draft: Draft<ListsState>, action: ListsActions) => {
    switch (action.type) {
      ...
      case ListsActionType.SET_UPDATED_LISTS:
        const updatedListIndex = draft.lists.findIndex(
      ...
      case ListsActionType.SET_FOLLOW_LIST:
        const followListIndex = draft.lists.findIndex(
          (list) => list.id === action.payload.id
        );
        if (followListIndex !== -1)
          draft.lists[followListIndex] = action.payload;
        draft.userLists = [...draft.userLists, action.payload];
        break;

      case ListsActionType.SET_UNFOLLOW_LIST:
        const unfollowListIndex = draft.lists.findIndex(
          (list) => list.id === action.payload.id
        );
        if (unfollowListIndex !== -1)
          draft.lists[unfollowListIndex] = action.payload;
        draft.userLists = draft.userLists.filter(
          (list) => list.id !== action.payload.id
        );
        draft.pinnedLists = draft.pinnedLists.filter(
          (list) => list.id !== action.payload.id
        );
        break;
      ...
      case ListsActionType.SET_PINED_LIST_TO_USER_LIST:
        const pinToUserListIndex = draft.userLists.findIndex(
          (list) => list.id === action.payload.id
        );
        if (pinToUserListIndex !== -1)
          draft.userLists[pinToUserListIndex] = action.payload;
        break;
