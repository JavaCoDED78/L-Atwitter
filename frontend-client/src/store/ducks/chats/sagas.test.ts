...
describe("chatsSaga:", () => {
  ...
  describe("leaveFromConversationRequest:", () => {
    const worker = leaveFromConversationRequest(
      leaveFromConversation({ participantId: 1, chatId: 1 })
    );

    testLoadingStatus(worker, setChatsLoadingState, LoadingStatus.LOADING);
    testCall(worker, ChatApi.leaveFromConversation, {
      participantId: 1,
      chatId: 1,
    });
    testLoadingStatus(worker, setChatsLoadingState, LoadingStatus.ERROR);
  ...
  testWatchSaga(chatsSaga, [
    ...
    {
      actionType: ChatsActionsType.LEAVE_FROM_CONVERSATION,
      workSaga: leaveFromConversationRequest,
    },
  ]);
});
