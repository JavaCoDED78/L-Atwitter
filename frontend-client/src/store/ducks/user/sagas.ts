...
export function* fetchFollowUserRequest({
  payload,
}: FollowUserActionInterface) {
  try {
...
export function* fetchUnfollowUserRequest({
  payload,
}: UnfollowUserActionInterface) {
  try {
...
export function* fetchStartUseTwitter({
  payload,
}: StartUseTwitterActionInterface) {
  try {
    yield put(setUserLoadingStatus(LoadingStatus.LOADING));
...
export function* fetchReadMessagesRequest({
  payload,
}: FetchReadMessagesActionInterface) {
  try {
    yield put(setTweetsLoadingState(LoadingStatus.LOADING));
