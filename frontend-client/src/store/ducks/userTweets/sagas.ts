...
export function* fetchUserLikedTweetsRequest({
  payload,
}: FetchUserLikedTweetsActionInterface) {
  try {
    yield put(setUserTweetsLoadingStatus(LoadingStatus.LOADING));
    const response: AxiosResponse<Tweet[]> = yield call(
      UserApi.getUserLikedTweets2,
      payload
    );
    yield put(
      setUserTweets({
...
export function* fetchUserMediaTweetsRequest({
  payload,
}: FetchUserMediaTweetsActionInterface) {
  try {
    yield put(setUserTweetsLoadingStatus(LoadingStatus.LOADING));
    const response: AxiosResponse<Tweet[]> = yield call(
      UserApi.getUserMediaTweets,
      payload
    );
    yield put(
      setUserTweets({
...
export function* fetchUserRetweetsAndReplies({
  payload,
}: FetchUserRetweetsAndRepliesActionInterface) {
  try {
    yield put(setUserTweetsLoadingStatus(LoadingStatus.LOADING));
    const response: AxiosResponse<Tweet[]> = yield call(
      UserApi.getUserRetweetsAndReplies,
      payload
    );
    yield put(
      setUserTweets({
...
export function* userTweetsSaga() {
  ...
  yield takeLatest(
    UserTweetsActionType.FETCH_LIKED_TWEETS,
    fetchUserLikedTweetsRequest
  );
  yield takeLatest(
    UserTweetsActionType.FETCH_MEDIA_TWEETS,
    fetchUserMediaTweetsRequest
  );
  yield takeLatest(
    UserTweetsActionType.FETCH_RETWEETS_AND_REPLIES,
