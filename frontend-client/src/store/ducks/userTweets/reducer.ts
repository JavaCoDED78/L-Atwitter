...
export const userTweetsReducer = produce(
  (draft: Draft<UserTweetsState>, action: UserTweetsActions) => {
    switch (action.type) {
      ...
      case UserTweetsActionType.SET_LIKED_TWEET:
        const tweetIndex = draft.items.findIndex(
      ...
      case UserTweetsActionType.SET_RETWEET:
        const retweetIndex = draft.items.findIndex(
          (tweet) => tweet.id === action.payload.id
        );
      ...
      case UserTweetsActionType.PIN_TWEET:
        if (action.payload.activeTab === 0) {
          let tweets = draft.items.filter(
            (tweet) => tweet.id !== action.payload.tweet.id
          );
