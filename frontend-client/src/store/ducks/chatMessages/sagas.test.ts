...
describe("chatMessagesSaga:", () => {
  describe("fetchChatMessagesRequest:", () => {
    ...
    testSetResponse({
      worker: worker,
      mockData: mockChatMessageResponse,
      action: setChatMessages,
      payload: mockChatMessageResponse,
      responseType: "ChatMessageResponse",
    });
    testLoadingStatus(worker, setChatMessagesLoadingState, LoadingStatus.ERROR);
  });

  describe("addChatMessageRequest:", () => {
    const mockChatMessageRequest = {
      chatId: 1,
      text: "text",
    } as ChatMessageRequest;
    const worker = addChatMessageRequest(
      addChatMessage(mockChatMessageRequest)
    );
  ...
  describe("addChatMessageWithTweetRequest:", () => {
    ...
    const worker = addChatMessageWithTweetRequest(
      addChatMessageWithTweet(mockChatMessageWithTweetRequest)
    );
    ...
    testCall(
      worker,
      ChatApi.addMessageWithTweet,
      mockChatMessageWithTweetRequest
    );
    testLoadingStatus(worker, setChatMessagesLoadingState, LoadingStatus.ERROR);
  });
  ...
  testWatchSaga(chatMessagesSaga, [
    {
      actionType: ChatMessagesActionsType.FETCH_CHAT_MESSAGES,
      workSaga: fetchChatMessagesRequest,
    },
    {
      actionType: ChatMessagesActionsType.ADD_CHAT_MESSAGE,
      workSaga: addChatMessageRequest,
    },
    {
      actionType: ChatMessagesActionsType.ADD_CHAT_MESSAGE_WITH_TWEET,
      workSaga: addChatMessageWithTweetRequest,
    },
  ]);
});
