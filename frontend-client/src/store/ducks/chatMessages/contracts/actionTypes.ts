...
export interface SetChatMessagesActionInterface
  extends Action<ChatMessagesActionsType> {
  type: ChatMessagesActionsType.SET_CHAT_MESSAGES;
  payload: ChatMessage[];
}

export interface SetChatMessageActionInterface
  extends Action<ChatMessagesActionsType> {
  type: ChatMessagesActionsType.SET_CHAT_MESSAGE;
  payload: ChatMessage;
}

export interface AddChatMessageActionInterface
  extends Action<ChatMessagesActionsType> {
  type: ChatMessagesActionsType.ADD_CHAT_MESSAGE;
  payload: ChatMessageRequest;
}

export interface AddChatMessageWithTweetActionInterface
  extends Action<ChatMessagesActionsType> {
  type: ChatMessagesActionsType.ADD_CHAT_MESSAGE_WITH_TWEET;
  payload: ChatMessageWithTweetRequest;
}

export interface FetchChatMessagesActionInterface
  extends Action<ChatMessagesActionsType> {
  type: ChatMessagesActionsType.FETCH_CHAT_MESSAGES;
  payload: number;
}

export interface SetChatMessagesLoadingStateActionInterface
  extends Action<ChatMessagesActionsType> {
  type: ChatMessagesActionsType.SET_LOADING_STATE;
  payload: LoadingStatus;
}

export interface ResetChatMessagesLoadingStateActionInterface
  extends Action<ChatMessagesActionsType> {
