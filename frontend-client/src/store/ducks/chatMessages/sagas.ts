...
export function* fetchChatMessagesRequest({
  payload,
}: FetchChatMessagesActionInterface) {
  try {
...
export function* addChatMessageRequest({
  payload,
}: AddChatMessageActionInterface) {
  try {
    setChatMessagesLoadingState(LoadingStatus.LOADING);
...
export function* addChatMessageWithTweetRequest({
  payload,
}: AddChatMessageWithTweetActionInterface) {
  try {
    setChatMessagesLoadingState(LoadingStatus.LOADING);
...
export function* chatMessagesSaga() {
  yield takeLatest(
    ChatMessagesActionsType.FETCH_CHAT_MESSAGES,
    fetchChatMessagesRequest
  );
  yield takeLatest(
    ChatMessagesActionsType.ADD_CHAT_MESSAGE,
    addChatMessageRequest
  );
  yield takeLatest(
    ChatMessagesActionsType.ADD_CHAT_MESSAGE_WITH_TWEET,
