...
export function* fetchTweetsByTagRequest({
  payload,
}: FetchTweetsByTagActionInterface) {
  try {
    yield put(setTweetsLoadingState(LoadingStatus.LOADING));
...
export function* fetchTweetsByTextRequest({
  payload,
}: FetchTweetsByTextActionInterface) {
  try {
    yield put(setTweetsLoadingState(LoadingStatus.LOADING));
...
export function* addScheduledTweetRequest({
  payload,
}: AddScheduledTweetActionInterface) {
  try {
...
export function* updateScheduledTweetRequest({
  payload,
}: UpdateScheduledTweetActionInterface) {
  try {
...
export function* changeReplyTypeRequest({
  payload,
}: ChangeReplyTypeActionInterface) {
  try {
...
export function* fetchDeleteTweetRequest({
  payload,
}: FetchDeleteTweetActionInterface) {
  try {
...
export function* deleteScheduledTweetsTweetRequest({
  payload,
}: DeleteScheduledTweetsActionInterface) {
  try {
...
export function* tweetsSaga() {
  ...
  yield takeLatest(
    TweetsActionType.FETCH_MEDIA_TWEETS,
    fetchMediaTweetsRequest
  );
  yield takeLatest(
    TweetsActionType.FETCH_TWEETS_WITH_VIDEO,
  ...
  yield takeLatest(
    TweetsActionType.ADD_SCHEDULED_TWEET,
    addScheduledTweetRequest
  );
  yield takeLatest(
    TweetsActionType.UPDATE_SCHEDULED_TWEET,
  ...
  yield takeLatest(
    TweetsActionType.DELETE_SCHEDULED_TWEETS,
  ...
  yield takeLatest(
    TweetsActionType.FETCH_TWEETS_BY_TAG,
    fetchTweetsByTagRequest
  );
  yield takeLatest(
    TweetsActionType.FETCH_TWEETS_BY_TEXT,
