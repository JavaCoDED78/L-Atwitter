...
export const selectUsersIsLoading = (state: RootState): boolean =>
  selectUsersState(state).loadingState === LoadingStatus.LOADING;
export const selectUsersLoadedSuccess = (state: RootState): boolean =>
  selectUsersState(state).loadingState === LoadingStatus.SUCCESS;
