...
export function* unfollowUseRequest({
  payload,
}: UnfollowUserProfileActionInterface) {
  try {
    const item: User = yield call(UserApi.follow, payload);
...
export function* processSubscribeRequest({
  payload,
}: ProcessSubscribeActionInterface) {
  try {
    const item: User = yield call(
      UserApi.processSubscribeToNotifications,
      payload
    );
    yield put(setUserProfile(item));
  } catch (error) {
...
export function* userProfileSaga() {
  ...
  yield takeLatest(
    UserProfileActionsType.PROCESS_SUBSCRIBE,
