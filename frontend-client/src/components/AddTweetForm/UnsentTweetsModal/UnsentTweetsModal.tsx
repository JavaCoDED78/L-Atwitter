...
const UnsentTweetsModal: FC<UnsentTweetsModalProps> = ({
  visible,
  onClose,
}): ReactElement | null => {
  ...
  const [visibleEditTweetModal, setVisibleEditTweetModal] =
    useState<boolean>(false);
  const [visibleEditListFooter, setVisibleEditListFooter] =
    useState<boolean>(false);
  ...
  const getScheduledTweets = (): void => {
    setTweets([]);
    setIsLoading(true);
    TweetApi.fetchScheduledTweets().then((response) => {
      setTweets(response);
      setIsLoading(false);
    });
  };
  ...
  const onToggleCheckTweet = (tweetId: string): void => {
    const currentIndex =
      checkboxIndexes.findIndex(
        (checkboxIndex) => checkboxIndex === tweetId
      ) !== -1;

    if (currentIndex) {
      setCheckboxIndexes(
        checkboxIndexes.filter((checkboxIndex) => checkboxIndex !== tweetId)
      );
    } else {
  ...
  const handleDeleteScheduledTweets = (): void => {
    if (checkboxIndexes.length !== 0) {
      TweetApi.deleteScheduledTweets({
        tweetsIds: checkboxIndexes.map((value) => Number(value)),
  ...
  return (
    ...
    >
      <DialogTitle id="form-dialog-title">
        <CloseButton
          onClose={!visibleEditTweetModal ? onClose : onCloseEditTweetModal}
        />
        ...
        {visibleEditTweetModal ? (
          ...
          <Button
            ...
            onClick={
              visibleEditListFooter ? onCloseEditTweetList : onOpenEditTweetList
            }
            type="submit"
            variant="contained"
            color="primary"
              ...
              >
                ...
                <div className={classes.tweetWrapper}>
                  <div className={classes.scheduledDateWrapper}>
                    ...
                    <Typography
                      component={"span"}
                      className={classes.scheduledDateText}
                    >
                      {`Will send on ${formatScheduleDate(
                        new Date(tweet.scheduledDate!)
                      )}`}
                    </Typography>
                  </div>
