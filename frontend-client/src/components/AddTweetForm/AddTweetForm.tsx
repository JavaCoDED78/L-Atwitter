...
const AddTweetForm: FC<AddTweetFormProps & SnackbarProps> = ({
  ...
  const [visibleAddMediaAction, setVisibleAddMediaAction] =
    useState<boolean>(false);
  const [visibleAddGifAction, setVisibleAddGifAction] =
    useState<boolean>(false);
  const [visibleAddPollAction, setVisibleAddPollAction] =
    useState<boolean>(false);
  const [visibleAddEmojiAction, setVisibleAddEmojiAction] =
    useState<boolean>(false);
  const [visibleAddScheduleAction, setVisibleAddScheduleAction] =
    useState<boolean>(false);
  const [visibleRemoveAction, setVisibleRemoveAction] =
    useState<boolean>(false);
  ...
  const handleHoverAction = (action: AddTweetFormAction): void => {
    if (action === AddTweetFormAction.MEDIA) {
      setDelayHandler(
        setTimeout(() => setVisibleAddMediaAction(true), HOVER_DELAY)
      );
    } else if (action === AddTweetFormAction.GIF) {
      setDelayHandler(
        setTimeout(() => setVisibleAddGifAction(true), HOVER_DELAY)
      );
    } else if (action === AddTweetFormAction.POLL) {
      setDelayHandler(
        setTimeout(() => setVisibleAddPollAction(true), HOVER_DELAY)
      );
    } else if (action === AddTweetFormAction.EMOJI) {
      setDelayHandler(
        setTimeout(() => setVisibleAddEmojiAction(true), HOVER_DELAY)
      );
    } else if (action === AddTweetFormAction.SCHEDULE) {
      setDelayHandler(
        setTimeout(() => setVisibleAddScheduleAction(true), HOVER_DELAY)
      );
    } else if (action === AddTweetFormAction.REMOVE) {
      setDelayHandler(
        setTimeout(() => setVisibleRemoveAction(true), HOVER_DELAY)
      );
    }
  };
          ...
          {buttonName !== "Reply" && (
            <div>
              ...
              >
                <>{ScheduleIcon}</>
                <HoverAction
                  visible={visibleAddScheduleAction}
                  actionText={"Schedule"}
                />
              </IconButton>
            </div>
