...
const TweetComponentActions: FC<TweetComponentActionsProps> = ({
  ...
  const isUserMuted =
    myProfile?.userMutedList?.findIndex(
      (mutedUser) => mutedUser.id === tweet.user.id
    ) !== -1;
  const isUserBlocked =
    myProfile?.userBlockedList?.findIndex(
      (blockedUser) => blockedUser.id === tweet.user.id
    ) !== -1;
  ...
  const onMuteUser = (): void => {
    ...
    setSnackBarMessage(
      `@${tweet.user.username} has been ${isUserMuted ? "unmuted" : "muted"}.`
    );
    setOpenSnackBar(true);
  };
              ...
              <List>
                {myProfile?.id === tweet.user.id ? (
                  <>
                    ...
                    <ListItem>
                      <>{EmbedTweetIcon}</>
                      <Typography component={"span"}>Embed Tweet</Typography>
                    </ListItem>
                  ...
                  <>
                    ...
                    <ListItem>
                      <>{ReportIcon}</>
                      <Typography component={"span"}>Report Tweet</Typography>
                    </ListItem>
                  </>
                )}
              </List>
