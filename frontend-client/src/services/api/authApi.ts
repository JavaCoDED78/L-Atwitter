...
export const AuthApi = {
  async signIn(
    ...
    const { data } = await axios.post<Response<AuthenticationResponse>>(
      `${API_URL}/auth/login`,
      postData
    );
    return data;
  ...
  async getUserByResetCode(resetCode: string): Promise<AuthUserResponse> {
    const { data } = await axios.get<AuthUserResponse>(
      `${API_URL}/auth/reset/${resetCode}`
    );
    return data;
  },
  ...
  async currentPasswordReset(postData: {
    currentPassword: string;
    password: string;
    password2: string;
  }): Promise<string> {
    const { data } = await axios.post<string>(
      `${API_URL}/auth/reset/current`,
      postData
    );
    return data;
  },
  async getMe(): Promise<AuthenticationResponse> {
    const { data } = await axios.get<AuthenticationResponse>(
      `${API_URL}/auth/user`
    );
    return data;
  },
