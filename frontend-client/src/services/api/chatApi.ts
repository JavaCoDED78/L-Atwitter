...
export const ChatApi = {
  ...
  async createChat(userId: number): Promise<Response<Chat>> {
    const data = await axios.get<Response<Chat>>(
      API_URL + "/chat/create/" + userId
    );
    return data.data;
  },
  async getChatMessages(chatId: number): Promise<Response<ChatMessage[]>> {
    const data = await axios.get<Response<ChatMessage[]>>(
  ...
  async readChatMessages(chatId: number): Promise<Response<User>> {
    const data = await axios.get<Response<User>>(
      API_URL + `/chat/${chatId}/read/messages`
    );
    return data.data;
  },
  async addMessage(
    ...
    const data = await axios.post<Response<ChatMessage>>(
      API_URL + "/chat/add/message",
      chatMessage
    );
    return data.data;
  },
  async addMessageWithTweet(
    chatMessage: ChatMessageWithTweetRequest
  ): Promise<Response<ChatMessage[]>> {
    const data = await axios.post<Response<ChatMessage[]>>(
      API_URL + "/chat/add/message/tweet",
      chatMessage
    );
    return data.data;
  },
