...
export const UserApi = {
  ...
  async getUserFollowers(userId: string): Promise<User[] | undefined> {
    const { data } = await axios.get<User[] | undefined>(
  ...
  async processSubscribeToNotifications(
    userId: number
  ): Promise<User | undefined> {
    const { data } = await axios.get<User | undefined>(
      `${API_URL}/user/subscribe/${userId}`
    );
    return data;
  },
  async getUserTweets(payload: {
    userId: string;
    page: number;
  }): Promise<AxiosResponse<Tweet[]>> {
    return await axios.get<Tweet[]>(
      `${API_URL}/user/${payload.userId}/tweets`,
      { params: { page: payload.page } }
    );
  },
  async getUserLikedTweets(payload: {
    userId: string;
    page: number;
  }): Promise<AxiosResponse<Tweet[]>> {
    return await axios.get<Tweet[]>(`${API_URL}/user/${payload.userId}/liked`, {
      params: { page: payload.page },
    });
  },
  async getUserMediaTweets(payload: {
    userId: string;
    page: number;
  }): Promise<AxiosResponse<Tweet[]>> {
    return await axios.get<Tweet[]>(`${API_URL}/user/${payload.userId}/media`, {
      params: { page: payload.page },
    });
  },
  async getUserRetweetsAndReplies(payload: {
    userId: string;
    page: number;
  }): Promise<AxiosResponse<Tweet[]>> {
    return await axios.get<Tweet[]>(
      `${API_URL}/user/${payload.userId}/replies`,
      { params: { page: payload.page } }
    );
  },
  async getUserNotifications(): Promise<Response<Notifications>> {
    const { data } = await axios.get<Response<Notifications>>(
      `${API_URL}/user/notifications`
    );
    return data;
  },
  async getNotificationsFromTweetAuthors(
    payload: number
  ): Promise<AxiosResponse<Tweet[]>> {
    return await axios.get<Tweet[]>(`${API_URL}/user/notifications/timeline`, {
      params: { page: payload },
    });
  },
  async getUserBookmarks(payload: number): Promise<AxiosResponse<Tweet[]>> {
    return await axios.get<Tweet[]>(`${API_URL}/user/bookmarks`, {
      params: { page: payload },
    });
  },
  ...
  async startUseTwitter(id: number): Promise<Response<User>> {
    const { data } = await axios.get<Response<User>>(
      `${API_URL}/user/${id}/start`
    );
    return data;
  },
  async pinTweet(tweetId: string): Promise<Response<User>> {
    const { data } = await axios.get<Response<User>>(
      `${API_URL}/user/pin/tweet/${tweetId}`
    );
    return data;
  },
