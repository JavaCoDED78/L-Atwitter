...
export const withHoverAction =
  <T extends object>(Component: React.ComponentType<HoverActionProps>) =>
  (props: HoverActionProps) => {
    const [visibleReplyAction, setVisibleReplyAction] =
      useState<boolean>(false);
    const [visibleRetweetAction, setVisibleRetweetAction] =
      useState<boolean>(false);
    ...
    const handleHoverAction = (action: TweetActions): void => {
      if (action === TweetActions.REPLY) {
        setDelayHandler(
          setTimeout(() => setVisibleReplyAction(true), HOVER_DELAY)
        );
      } else if (action === TweetActions.RETWEET) {
        setDelayHandler(
          setTimeout(() => setVisibleRetweetAction(true), HOVER_DELAY)
        );
      } else if (action === TweetActions.LIKE) {
        setDelayHandler(
          setTimeout(() => setVisibleLikeAction(true), HOVER_DELAY)
        );
