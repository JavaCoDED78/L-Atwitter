...
export const withHoverAction =
  <T extends HoverActionProps>(Component: ComponentType<T>) =>
  (props: T) => {
    const initialState = {
    ...
    const [visibleHoverAction, setVisibleHoverAction] =
      useState<VisibleActions>({ ...initialState });
    ...
    const setHoverAction = (name: VisibleActions): void => {
      setDelayHandler(
        setTimeout(() => setVisibleHoverAction(name), HOVER_DELAY)
      );
    };
